  ...
  try {
    ...
    let add_hashtag = async function (element) {
      try {
        ...
        if (hashtag_found) { // 해시태그 테이블에 등록된적이 있는 해시태그면
          let hashtag_updated = await hashtag.update({ count: hashtag_found.dataValues.count + 1 }, { where: { name: element } }); // 해시태그 테이블에서 카운트 + 1 한다.
          // hashtag_updated 에는 업데이트 된 로우의 업데이트 된 속성만 가지고 온다.
          // await hashtag_found.setPosts(last_inserted_post.id); // Posthashtag 테이블에 post테이블의 id와 hashtag테이블의 hashtagname을 값으로 가지는 로우를 생성함.
          await posthashtag.create({ post_id: last_inserted_post.dataValues.id, hashtag_name: element });
        } else {  // 해시태그 테이블에 등록된 적이 없는 해시태그면 
          let hashtag_created = await hashtag.create({ name: element }); //hashtag 테이블에 로우 생성하고 기본 값은 1이 된다(모델 정의).
          // let isSaved = await hashtag_created.setPosts(last_inserted_post.id); // Posthashtag 테이블에 post테이블의 id와 hashtag테이블의 hashtagname을 값으로 가지는 로우를 생성함.
          await posthashtag.create({ post_id: last_inserted_post.dataValues.id, hashtag_name: element });
          // await console.log('setPosts saved value: ', isSaved);
        }
      } catch (err) {  // 왜 에러를 잡았을 때 이쪽으로 가지 못하나.
    ...
    if (Array.isArray(req.body.hashtag)) {
      await Promise.all(req.body.hashtag.map(el => new Promise((resolve, reject) => {
        resolve();
